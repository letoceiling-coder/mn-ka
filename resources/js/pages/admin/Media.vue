        ...
        <div v-if="mediaFiles.length > 0 && !loadingMedia" class="pt-6">
          ...
          <div class="grid gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            <div
              ...
              @click="(e) => handleFileClick(file, e)"
            >
              <!-- Превью изображения -->
              <div class="relative aspect-video bg-muted/30 overflow-hidden flex-shrink-0">
                ...
                <div
                  v-else
                  class="w-full h-full flex flex-col items-center justify-center bg-muted/50 p-4 pointer-events-none"
                >
                  <div class="text-5xl mb-2">
                    {{ getFileIcon(file) || '📎' }}
                  </div>
                ...
                <div
                  ...
                  class="absolute top-2 left-2 w-6 h-6 rounded-full bg-primary text-white flex items-center justify-center text-xs font-bold z-10 pointer-events-none"
                >
                  ✓
                </div>
                <!-- Overlay для режима выбора -->
                <div
                  ...
                  :class="[
                    ...
                    isFileSelected(file) ? 'opacity-100 bg-primary/30' : 'opacity-0 group-hover:opacity-100 bg-primary/20'
                  ]"
                >
                  <div class="w-16 h-16 rounded-full backdrop-blur-sm flex items-center justify-center shadow-lg transition-transform bg-primary/95 text-white">
              ...
              <!-- Информация о файле -->
              <div 
                class="p-3 space-y-1 flex-grow"
                :class="{ 'pointer-events-none': selectionMode && (file.type === 'photo' || file.type === 'video') }"
              >
              ...
              <div 
                v-if="!selectionMode" 
                class="px-3 pb-3 flex gap-2 mt-auto"
              >
                <!-- Кнопки для обычных папок -->
                <template v-if="!isTrashFolder">
                  <!-- Скачать -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleFileClick = (file, event) => {
      console.log('[Media] handleFileClick called:', { file, selectionMode: props.selectionMode, fileType: file?.type });
      
      // Если включен режим выбора, обрабатываем выбор файла
      if (props.selectionMode && file && (file.type === 'photo' || file.type === 'video')) {
        console.log('[Media] Selection mode: selecting file:', file);
        event?.stopPropagation();
        emit('file-selected', file);
        return;
      }
      
      // Если не режим выбора, открываем просмотр
    ...
    const openFilePreview = (file) => {
      ...
      if (props.selectionMode) {
        console.log('[Media] Selection mode: emitting file-selected event for file:', file);
        emit('file-selected', file)
        return
      }
