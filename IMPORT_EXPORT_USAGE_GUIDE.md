...
> **Примечание:** Поля со звездочкой (*) являются обязательными для заполнения.
...
3. **Звездочка (*)** в заголовке обозначает обязательное поле
4. **Пути к изображениям** должны быть относительными от корня архива: `images/products/image.jpg`
5. **ID через запятую** без пробелов: `1,2,3`, а не `1, 2, 3`
6. **Булевы значения**: используйте `1` для true, `0` для false
